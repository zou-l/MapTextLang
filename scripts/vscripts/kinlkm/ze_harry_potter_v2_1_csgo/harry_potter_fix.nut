...
function MapStart()
{
    ...
    if(STAGE == 0 && !ONLYZMMOD)
	{
	    ...
	    EntFire("console", "Command", "sm_rcon mp_restartgame 1", 117.00, null);
...
function StartTextHud()
{
    if(TimerTick)
    {
        ...
        for(local au = 1; au < 2; au++)
        {
            if(au == 1)
            {
                ScriptPrintMessageCenterAll("<font color='#00FFFF'>"+TimerText+""+min2+""+min1+":"+"</font>"+"<font color='#00FFFF'>"+sec2+""+sec1+"</font>");
                Time--;
            }
            if(Time == -1)
            {
